...
for filename in os.listdir(ccd_folder):
    ...
    try:
        ...
        print('got angles with ffi number:', ffi_num)
    except:
...
X = np.array(X)[:-1]
Y = np.array(Y)[:-1].reshape((len(X), 16, 16, 1))
ffis = np.array(ffis)[:-1]
...
model = Glow(steps=1, levels=3, img_shape=(16, 16, 1), hidden_channels=512, perm_type="1x1")
